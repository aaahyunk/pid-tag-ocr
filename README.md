...
## HITL 웹 URL : https://aaahyunk.github.io/pid-tag-ocr/
...
### [1] [청사진]PID_OCR.ipynb
   - (** 코드 산출물인 '청사진'을 시연할 수 있는 데이터 파일은 미리 구축하여 '웹_시연_데이터'에 있으므로 따로 실행하지 않으셔도 됩니다.)
   - [input : 구글 공유 드라이브의 '샘플_원천데이터', '샘플_라벨링데이터']
   - [output : 각 도면의 문법 사전 데이터 cache 파일(csv)]

[사용방법]
1. 구글 Colab T4 GPU 환경 설정 및 구글 드라이브 마운트
2. 코드 실행 후 /content/pid_out/cache 에서 csv 파일 다운로드

====================================================================================

### [2] [청사진]_P&ID 도면 태그 추출 자동화 및 자기지도 OCR HITL 웹 URL :
  https://aaahyunk.github.io/pid-tag-ocr/

  - [input : [청사진]PID_OCR.ipynb 에서 얻은 도면 cache 파일(csv)]
  - [output : 최종 도면 태그 텍스트 추출 데이터 'pid_tags_decoded.csv']

[사용방법]
1. URL 접속
2. 'CSV 업로드' 버튼 클릭 후 구글 공유 드라이브의 '웹_시연_데이터' 폴더의 csv 파일 중 1개 업로드
3. 하단 리스트에서 '검토 큐 열기' 버튼 클릭하여 '승인' 처리
4. '결과 다운로드'로 최종 수정된 태그 csv 파일 저장

====================================================================================


### [3] [청사진]_구글 공유 드라이브 링크 :
   https://drive.google.com/drive/folders/1qLvuKWBy10LOiqEZpt20eGQHICxtf1kE?usp=share_link
...
> [표1]과 [표2]의 n이 다른 이유: [표1]은 82건 전체(반려 포함)를 대상으로 한 운영 지표이고,
> [표2]는 사람이 실제로 검수해야 하는 18건을 제외한 64건 서브셋에서만 계산 가능한 정확도 지표입니다.
> 나머지 18건은 아직 사람 검토가 끝나지 않은 blank(REVIEW) 상태라 [표2] 분모에서 제외됩니다.
